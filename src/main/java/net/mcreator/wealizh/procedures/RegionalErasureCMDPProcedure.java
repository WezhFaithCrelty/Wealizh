...
public class RegionalErasureCMDPProcedure {
	public static void execute(LevelAccessor world, CommandContext<CommandSourceStack> arguments, Entity entity) {
		if (entity == null)
			return;
		RegionalErasurePProcedure.execute(world, commandParameterBlockPos(arguments, "origin").getX(), commandParameterBlockPos(arguments, "origin").getY(), commandParameterBlockPos(arguments, "origin").getZ(),
				DoubleArgumentType.getDouble(arguments, "pow"));
		if (world instanceof ServerLevel _serverLevelGR4 && _serverLevelGR4.getGameRules().getBoolean(GameRules.RULE_SENDCOMMANDFEEDBACK)) {
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal(((new ItemStack(WealizhModItems.LANGUAGE_TESTER.get()).getDisplayName().getString()).equals("[\u6587]")
						? "\u5DF2\u5728" + commandParameterBlockPos(arguments, "origin").getX() + " " + commandParameterBlockPos(arguments, "origin").getY() + " " + commandParameterBlockPos(arguments, "origin").getZ()
								+ "\u5904\u91CA\u653E\u4E86\u533A\u57DF\u62B9\u9664"
						: "The regional erasure has been released " + commandParameterBlockPos(arguments, "origin").getX() + " " + commandParameterBlockPos(arguments, "origin").getY() + " " + commandParameterBlockPos(arguments, "origin").getZ())),
						false);
		}
	}
